...
from tensorflow.keras.layers import Input, LSTM, Concatenate, TimeDistributed, GlobalAveragePooling1D, Embedding, \
    Dense, RepeatVector, Bidirectional, Conv1D
...
def build_encoder(n_inputs, n_features, encoder_lstm_units: List[int]):
    ...
    x = Conv1D(filters=16, kernel_size=3, name=f'encoder_CNN_0')(x)
    x = Conv1D(filters=16, kernel_size=3, name=f'encoder_CNN_1')(x)
    for idx, lstm_units in enumerate(encoder_lstm_units):
        lstm = LSTM(lstm_units, activation='relu', return_state=True, return_sequences=True, dropout=0.1,
                    recurrent_dropout=0.1, name=f'encoder_LSTM_{idx}')
        x, forward_h, forward_c, backward_h, backward_c = Bidirectional(lstm, name=f'encoder_BiLSTM_{idx}')(x)

    state_h = Concatenate()([forward_h, backward_h])
    state_c = Concatenate()([forward_c, backward_c])

    return inputs_layers, state_h, state_c


def build_decoder(state_h, state_c, decoder_dense_units, decoder_lstm_units: List[int], decoder_cat_dict):

    inputs_layers = []

    categorical_inputs = [RepeatVector(7)(state_h)]
...
def build_model(n_inputs, n_features, decoder_cat_dict: Dict, encoder_lstm_units: List[int],
    ...
    encoder_inputs_layers, state_h, state_c = build_encoder(n_inputs, n_features, encoder_lstm_units)

    decoder_inputs_layers, outputs = build_decoder(state_h, state_c, decoder_dense_units=decoder_dense_units,
